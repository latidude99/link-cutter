...
public class H2Controller {
    ...
    //    public static final String URL_BASE = "http://localhost:8080/"// injected from properties instead
...
*/
    ...
    public RedirectView redirectToOriginal(@PathVariable String id) {
        ...
        if ("".equals(original)) {
            ...
            return new RedirectView(
                    appProperties.getUrlBase() + "info" + id.substring(id.lastIndexOf("/") + 1));
        } else {
    ...
    public String showLinkInfo(@ModelAttribute LinkDTOH2 linkDTOH2, Model model) {
        if (linkDTOH2.getShortened().contains(appProperties.getUrlBase())) {
            String id = linkDTOH2.getShortened()
                    .substring(linkDTOH2.getShortened().lastIndexOf("/") + 1);
            LinkDTOH2 link = linkServiceH2.getOriginalLinkInfo(id);
            model.addAttribute("linkDTOH2ToDelete", linkDTOH2ToDelete);
            ...
            model.addAttribute("unrecognised",
                    "All links created with this service should start with " +
                            appProperties.getUrlBase());
        }
        return "h2info";
    }
    ...
    public String deleteLink(@ModelAttribute LinkDTOH2 linkDTOH2ToDelete, Model model) {
        ...
        if (linkDTOH2ToDelete.getShortened().contains(appProperties.getUrlBase())) {
